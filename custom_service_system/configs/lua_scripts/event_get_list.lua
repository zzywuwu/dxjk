...
local function Execute(post)
	...
	local _query_sql = "select record.*,customer.name as customer_name from record,customer where (customer.id = record.customer_id and customer.vip < 2 and record.visit_date >= now() and record.status = 0) order by record.visit_time desc"
