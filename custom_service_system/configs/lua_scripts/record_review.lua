...
local function Execute(post)
	...
	if (_review_content == '') then
		_query_sql = "update record set review_time = '0000-00-00 00:00:00', review_content = "..ngx.quote_sql_str(_review_content).." where id = "..ngx.quote_sql_str(_id)
	else
		_query_sql = "update record set review_time = NOW(), review_content = "..ngx.quote_sql_str(_review_content).." where id = "..ngx.quote_sql_str(_id)
	end

	DEBUG("record_review: " .. _query_sql)
